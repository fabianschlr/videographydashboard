...
export function SettingsClient({ userId, initial }: { userId: string; initial: { energy_default?: string; accent_color?: string; theme?: string; working_hours_per_week?: number } }) {
  ...
  async function save() {
    const { error } = await supabase.from("settings").upsert({ user_id: userId, ...settings }, { onConflict: "user_id" });
    setNotice(error ? "Speichern fehlgeschlagen. Bitte erneut versuchen." : "Gespeichert.");
  }
  async function importCsv(file: File) {
    ...
    const { error: deleteError } = await supabase.from("shifts").delete().eq("user_id", userId).gte("date", dates[0]).lte("date", dates[dates.length - 1]);
    const { error: insertError } = deleteError ? { error: deleteError } : await supabase.from("shifts").insert(shifts);
    setNotice(insertError ? "Import fehlgeschlagen. Prüfe das CSV-Format." : `${shifts.length} Schichten importiert.`);
  }
