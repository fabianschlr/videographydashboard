...
export function SettingsClient({ userId, initial }: { userId: string; initial: { energy_default?: string; accent_color?: string; theme?: string; working_hours_per_week?: number } }) {
  ...
  async function importCsv(file: File) {
    ...
    const { error } = await supabase.rpc("replace_shifts", { p_shifts: shifts });
    setNotice(error ? "Import fehlgeschlagen. Prüfe das CSV-Format." : `${shifts.length} Schichten importiert.`);
  }
